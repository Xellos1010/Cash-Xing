...
namespace BoomSports.Prototype.Managers
{
    ...
    public class UITextManager : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            matrix.managers.machineInfoManager.newMultiplier += Machine_information_manager_new_multiplier_set;
            matrix.managers.machineInfoManager.newBetAmount += Machine_information_manager_new_bet_amount;
        ...
        private void StateManager_StateChangedTo(States State)
        {
            switch (State)
            {
                case States.Idle_Intro:
                    if (matrix.managers.machineInfoManager.machineInfoScriptableObject.freespins <= 0)
                    {
