...
namespace Slot_Engine.Matrix
{
    ...
    public partial class EndConfigurationManager : MonoBehaviour
    {
        ...
        private SlotDisplaySymbol[] GenerateEndingReelStrip(GameModes mode, ref ReelStripManager reelStripManager)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public SlotDisplaySymbol GetRandomWeightedSymbol(GameModes currentMode)
        {
            SlotDisplaySymbol output = new SlotDisplaySymbol();
            int symbol = matrix.DrawRandomSymbol(StateManager.enCurrentMode);
            if (matrix.isSymbolOverlay(symbol))
            {
                ...
                {
                    symbol = matrix.DrawRandomSymbolFromCurrentState();
                }
            }
